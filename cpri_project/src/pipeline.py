...
# --------------------------------------------------------------------------
TEAM_NAME = "KDG"                       # -> used to name the output CSV
...
def build_predictions(train_df, test_df, clf, reg):
    ...
    result = pd.DataFrame({
        ...
        "Valid / Invalid": np.where(final_valid, "Valid", "Invalid"),
    })

    diagnostics = test_df.copy()
    diagnostics["ml_prob_valid"] = prob_valid.round(4)
    diagnostics["final_validity"] = result["Valid / Invalid"]
...
def build_summary(result, diagnostics, importances):
    n_total = len(result)
    n_invalid = int((result["Valid / Invalid"] == "Invalid").sum())
